/**
 * Calculate confidence score for XSS reports to reduce false positives
 * 
 * This module implements multiple heuristics to determine the likelihood
 * that a reported payload execution is a genuine XSS vulnerability rather
 ...
 */
async function calculateConfidenceScore(reportData) {
  ...
  // 2. Analyze URL characteristics
  heuristicScore += analyzeUrl(reportData.url);
  ...
  // 70% heuristic, 30% ML
  const finalScore = Math.round(heuristicScore * 0.7 + mlScore * 0.3);
