/**
 * Duplicate detection for Delen Private Bank XSS reports
 * 
 * This module identifies and filters duplicate XSS reports with
 * specialized logic for financial institutions.
 */
 ...
 */
async function detectDelenDuplicate(reportData) {
  ...
  const currentHash = generateDelenDuplicateHash(reportData);
  
  // Calculate the time window for duplicate detection
  const timeWindow = config.security.duplicateDetectionWindow || 600; // Default 10 minutes for banking
 ...
 */
function generateDelenDuplicateHash(reportData) {
  ...
  const data = {
    ...
    ip: reportData.ip,
    // Include banking-specific fields for more accurate deduplication
...
function normalizeUrlForComparison(url) {
  ...
  try {
    ...
    // For banking applications, we want to be more careful about which parameters we remove
    // Some parameters might be important for identifying different states of the application
    const bankingParams = [
      'account', 'transaction', 'payment', 'transfer'
    ];
    ...
    sortedParams.forEach(([key, value]) => {
      // Keep banking-related parameters
      if (!bankingParams.some(bp => key.includes(bp))) {
...
/**
 * Advanced duplicate detection using fuzzy matching for banking context
 * @param {Object} reportData - The data from the payload execution
 * @returns {Promise<boolean>} True if duplicate, false otherwise
 */
async function detectDelenFuzzyDuplicate(reportData) {
  // Calculate the time window for duplicate detection
  const timeWindow = config.security.duplicateDetectionWindow || 600; // Default 10 minutes
  const cutoffTime = new Date(Date.now() - timeWindow * 1000);
  ...
  const recentReports = await Report.find({
    createdAt: { $gte: cutoffTime }
  }).limit(20); // Increase limit for banking context
    ...
    if (urlDomain === reportDomain) {
      ...
      if (similarStrings(reportData.userAgent, report.userAgent, 0.8)) {
        ...
        if (similarIP(reportData.ip, report.ip)) {
          // Similar banking data
          if (similarBankingData(reportData.bankingData, report.bankingData)) {
            return true;
          }
        }
      }
...
/**
 * Check if banking data is similar
 * @param {Object} data1 - First banking data
 * @param {Object} data2 - Second banking data
 * @returns {boolean} True if banking data is similar
 */
function similarBankingData(data1, data2) {
  if (!data1 || !data2) return false;
  if (data1 === data2) return true;
  
  // Compare banking data fields
  const fieldsToCompare = ['accountFields', 'amountFields', 'beneficiaryFields', 'sensitiveKeywords'];
...
module.exports = {
  detectDelenDuplicate,
  detectDelenFuzzyDuplicate
};